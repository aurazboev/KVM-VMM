    ...
    while (1) {
        ...
        if (run->exit_reason == KVM_EXIT_IO) {
            ...
            if (d == KVM_EXIT_IO_OUT && p == CONSOLE_IO_PORT && s == 1 && c == 1) {
                ...
                sleep(1);
            }else if (d == KVM_EXIT_IO_IN && p == KEYBOARD_IO_PORT && s == 1 && c == 1){
                if(!key_handled){
            ...
            }else if (d == KVM_EXIT_IO_IN && p == KEYBOARD_STATUS_PORT && s == 1 && c == 1){
                printf("Debug: Setting status byte\n");
                uint8_t *reg = (uint8_t *)run + run->io.data_offset;
                *reg = 1;
            }
