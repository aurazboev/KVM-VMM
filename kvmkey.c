...
#define KEYBOARD_IO_PORT 0x44
#define KEYBOADR_STATUS_PORT 0x45
char console_buffer[1024];
size_t console_buffer_index = 0;

// Function declarations for enabling/disabling raw mode
...
void handle_console_io(uint8_t data) {
    printf("Debug: Writing to console: %c (0x%X)\n", data, data);
    if (data == 0x0A) {
...
int main(int argc, char *argv[]) {
    ...
    if (flags == -1) err(1, "fcntl get");
    flags |= O_NONBLOCK;
    if (fcntl(STDIN_FILENO, F_SETFL, flags) == -1) err(1, "fcntl set");

    char input_buf[1]; // Small buffer for single-character input
    ssize_t num_read;

    while (1) {
        num_read = read(STDIN_FILENO, input_buf, sizeof(input_buf));
        
        ret = ioctl(vcpufd, KVM_RUN, NULL);
        if (ret == -1) err(1, "KVM_RUN");
        if (run->exit_reason == KVM_EXIT_IO) {
            int d = run->io.direction, p = run->io.port, s = run->io.size, c = run->io.count;
            //printf("EXIT_io direction: %i, port: %x, size: %i, count: %i\n",d,p,s,c);
            if (d == KVM_EXIT_IO_OUT && p == CONSOLE_IO_PORT && s == 1 && c == 1) {
                uint8_t data = *(((uint8_t *)run) + run->io.data_offset);
                handle_console_io(data);
                sleep(5);
            }else if (d == KVM_EXIT_IO_IN && p == KEYBOADR_STATUS_PORT && s == 1 && c == 1){
                printf("Debug: Reading keyboard: %c (0x%X)\n", input_buf[0], input_buf[0]);
