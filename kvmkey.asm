...
int main(int argc, char *argv[]) {
    ...
    while (1) {
        ...
        if (num_read > 0) {
            // Handle the input. For example, print it or store it for your keyboard simulation
            printf("You pressed: %c\n", input_buf[0]);
            // Simulate key press here as needed
        }
        ret = ioctl(vcpufd, KVM_RUN, NULL);
        if (ret == -1) err(1, "KVM_RUN");
        if (run->exit_reason == KVM_EXIT_IO) {
            printf("%i",run->io.direction);
            ...
            }else if (run->io.direction == KVM_EXIT_IO_IN && run->io.port == KEYBOADR_STATUS_PORT && run->io.size == 1 && run->io.count == 1){
                printf("Debug: Inside read: %c (0x%X)\n", input_buf[0], input_buf[0]);
